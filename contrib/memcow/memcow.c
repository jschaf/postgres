 ...
 *
 * memcow is a test-mode storage manager.  When the memcow.enabled GUC is on,
 * smgropen() selects memcow instead of md for every relation, and relation
 * pages are served from a read-only PGDATA seed (memcow.seed_directory) plus
 ...
 * against memcow.seed_directory and never against DataDir.  That is why the
 ...
 * a fork is memcow.seed_directory concatenated with the *relative* path
 ...
 * postmaster (memcow.enabled and memcow.seed_directory are both
...
struct MemcowRelEntry
{
	...
	LWLock lock;				/* protects fields below; entry never moves */
 ...
 * state and nonce that the login event trigger checks, the registry of pool backend
...
MemcowShmemInit(void)
{
	...
	MemcowShmem = ShmemInitStruct("memcow overlay directory",
								 sizeof(MemcowShmemState), &found);
	if (!found)
	{
...
void
memcow_shmem_setup(void)
{
	prev_shmem_request_hook = shmem_request_hook;
	shmem_request_hook = MemcowShmemRequest;
	prev_shmem_startup_hook = shmem_startup_hook;
	shmem_startup_hook = MemcowShmemInit;
}
 ...
 * registered storage manager whether or not it is the selected one.  So the
 * very first thing to do is to get out of the way when memcow is off: with
 * memcow.enabled off this function must be a perfect no-op, because that is
...
memcow_check_fingerprint(void)
{
		...
		ereport(FATAL,
				(errcode_for_file_access(),
				 errmsg("could not open memcow seed fingerprint \"%s\": %m",
						path),
				 errhint("memcow.seed_directory must name a seed built by src/test/memcow/seed/build_seed.sh.")));
	...
	{
		char module[MAXPGPATH];

		snprintf(module, sizeof(module), "%s/memcow%s", pkglib_path, DLSUFFIX);
		if (stat(module, &st) != 0)
			ereport(FATAL, (errcode_for_file_access(),
							errmsg("could not stat memcow module \"%s\": %m", module)));
		memcow_fingerprint_expect(image, path, "memcow_module_bytes", (int64) st.st_size);
	}
...
/*
 * Validate memcow.seed_directory.
 *
...
memcow_check_seed_directory(void)
{
	...
	if (memcow_seed_directory == NULL || memcow_seed_directory[0] == '\0')
		ereport(FATAL,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("memcow.enabled requires memcow.seed_directory to be set"),
				 errhint("Set memcow.seed_directory to the read-only PGDATA seed built for this server binary.")));

	if (!is_absolute_path(memcow_seed_directory))
		ereport(FATAL,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
				 errmsg("memcow.seed_directory must be an absolute path, not \"%s\"",
						memcow_seed_directory),
 ...
 *
 * The optional release-all callback runs even when the process has no open
 * relations, e.g. a checkpointer after smgrdestroyall(). It must not allocate.
 */
void
...
/*
 * Partition locks protect membership; per-entry locks protect mutable fields.
 * Relation entries are never deleted or moved until the whole arena detaches.
 * Lock order is partition then entry. No holder of an entry lock walks rels.
 * Truncate uses the already mapped pointer and takes only the entry lock.
 */
static MemcowRelEntry *
memcow_find_rel(MemcowDbLocal *db, const MemcowRelKey *key, bool exclusive)
{
	MemcowRelEntry *re = dshash_find(db->rels, key, false);

	if (re != NULL)
	{
		LWLockAcquire(&re->lock, exclusive ? LW_EXCLUSIVE : LW_SHARED);
		dshash_release_lock(db->rels, re);
	}
	return re;
}

/* Copy fork metadata under its entry lock, then resolve seed blocks. */
static void
				   ...
				   MemcowFork *f)
{
	...
	if (f->db != NULL)
		re = memcow_find_rel(f->db, &f->relkey, false);
	else
		re = NULL;
					 ...
					 RelFileNumber relNumber)
{
	...
	/*
	 * ERRCODE_MEMCOW_ARENA_FULL whether the ceiling was memcow.lane_arena_limit
	...
	ereport(ERROR,
			(errcode(ERRCODE_MEMCOW_ARENA_FULL),
			 ...
			 errdetail("The arena holds %zu bytes; memcow.lane_arena_limit is %d MB. Failed while storing block %u of relation %u/%u.",
					   ...
					   relNumber),
			 errhint("Every page written in this epoch stays in the arena until the lane is reset; reset the lane, or raise memcow.lane_arena_limit.")));
}

/*
 * Find, or create, the overlay record for one fork, returning it with its
 * per-entry lock held exclusively.  The caller must release it.
 *
					 ...
					 MemcowDbLocal **dbp, bool *created)
{
	...
	*dbp = db;
	...
	re = memcow_find_rel(db, &key, true);
	if (re != NULL)
	{
		*created = false;
		return re;
	...
	if (!found)
	{
		LWLockInitialize(&re->lock, MemcowShmem->rel_tranche);
		re->exists = fs->exists;
	...
	*created = !found;
	LWLockAcquire(&re->lock, LW_EXCLUSIVE);
	dshash_release_lock(db->rels, re);
	return re;
}
 ...
 * re-lock its LWLock without walking the table at all.
...
memcow_tablespace_in_use(Oid spcOid)
{
	...
	for (int i = 0; i < ndbs; i++)
	{
		...
		{
			LWLockAcquire(&re->lock, LW_SHARED);
			found = re->exists && re->key.spcOid == spcOid;
			LWLockRelease(&re->lock);
			if (found)
				break;
		}
 ...
 * the published overlay, the admission state and nonce that the login event trigger
 ...
 * SQL surface for all of this is contrib/memcow; nothing here is
 * reachable without it except the admission check.
 *
 * The epoch and arena handles are published together under the directory
 * lock. Both the data plane and the lane control plane live in this module;
 * lanes.c supplies preload hooks and catalog-backed SQL entry points.
 * ----------------------------------------------------------------
 */
 ...
 */
/* memcow.lane_arena_limit, in the unit dsa_set_size_limit() wants */
static size_t
...
memcow_lane_open(Oid dbOid, bool arm, const char *datname)
{
	...
	/*
	 * Bind memcow.lane_arena_limit to the published arena.  A reset's PREPARE
...
/*
 * memcow_check_admission() -- called by the login event trigger after InitPostgres().
 *
 ...
 * connection to have presented the nonce via the memcow.lane_nonce GUC
...
memcow_check_admission(void)
{
	 ...
	 * contrib/memcow refuses with the same message shapes, and a test
	...
	if (state != MEMCOW_LANE_OPEN)
		ereport(FATAL,
				(errcode(ERRCODE_CANNOT_CONNECT_NOW),
				 ...
				 errdetail("Refused by the login event trigger admission fence (plan §5 I2, fence 3 of 3).")));
		...
		ereport(FATAL,
				(errcode(ERRCODE_CANNOT_CONNECT_NOW),
				 errmsg("memcow lane nonce mismatch for database %u",
						MyDatabaseId),
				 errdetail("The connection presented nonce %d; refused by the login event trigger admission fence (plan §5 I2, fence 3 of 3).",
						   memcow_lane_nonce)));
}
