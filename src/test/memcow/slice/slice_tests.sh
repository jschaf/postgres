...
#
# Persistent-session cases use PostgreSQL::Test::Cluster and background psql
# in t/001_reset.pl. This entry point retains case/control selection and the
# seed/RAM preconditions shared with the other slices.
# ===========================================================================
...
# Run persistent-session slices through PostgreSQL's existing TAP facilities.
# Each invocation owns one server lifetime and leaves its TAP/session logs.
tap_case()
{
	local name=$1 tapdir perl_bin
	pg_running && pg_stop fast
	mc_ensure_startable "$PGDATA" "$SEED" "$RAM_MOUNT" "$OUTPUTDIR" || return 1
	tapdir=$(mktemp -d "$OUTPUTDIR/$name.tap.XXXXXX") || return 1
	perl_bin=${MEMCOW_PERL:-perl}
	if [ -z "${MEMCOW_PERL:-}" ] && [ -x /opt/homebrew/opt/perl/bin/perl ]; then
		perl_bin=/opt/homebrew/opt/perl/bin/perl
	fi
	if PATH="$MC_BINDIR:$PATH" LANG=C LC_CTYPE=C LC_COLLATE=C \
		TESTDATADIR="$tapdir/data" TESTLOGDIR="$tapdir/log" \
		MEMCOW_TAP_SERVER_LOG="$LOGFILE" PG_TEST_NOCLEAN=1 \
		"$perl_bin" -I "$HERE/../../perl" "$HERE/t/001_reset.pl" \
		"$name" "$NEGATIVE" "$PGDATA" "$SEED" "$DB" "$CONTROL_DB" "${PGUSER:-$(id -un)}" \
		>"$tapdir/tap.log" 2>&1
	then
		cat "$tapdir/tap.log"
	else
		cat "$tapdir/tap.log"
		ck "$name TAP case (see $tapdir)" 1
	fi
	ck_no_crash
}
...
for c in "${CASES[@]}"; do
	...
	CASE_FAIL=0
	case $c in
	S11_*|S13_*|S14_*|S18_*|R1_*|R2_*|R3_*|R4_*|R5_*)
		tap_case "$c" || ck "$c TAP launch" 1 ;;
	*)
